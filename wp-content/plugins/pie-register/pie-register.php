	...
	class PieRegister extends PieReg_Base{
		...
		*/
		...
		function piereg_register_scripts(){
			...
			wp_register_script('pie_datepicker_js',plugins_url("/js/datepicker.js",__FILE__),'jquery-ui-core','2.0',false);
		...
		function print_multi_lang_script_vars(){
			...
			$this->pie_pr_dec_vars_array = array(
				...
				'reCaptcha_language'					=> $opt['piereg_recaptcha_language'],
		...
		*/
		...
		function add_pie_form_button($context)
		{
			...
			$out = '<a href="#TB_inline?width=480&inlineId=select_pie_form" class="thickbox button" id="add_pie_form" title="' . __("Add Pie Register Form", 'pie-register') . '" ><span style="background: url('.plugins_url('pie-register').'/images/form-icon.png); background-repeat: no-repeat; background-position: left bottom;" class="wp-media-buttons-icon"></span> '.__("Add Form","pie-register").'</a>';
			return $context . $out;
		}
...
?>
		...
		function AddPanel()
		{
			...
			*/
			...
			$pie_page_suffix_11 = add_users_page( 'Unverified Users', __('Unverified Users',"pie-register") , 'manage_options', 'unverified-users', array($this, 'Unverified') );
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				*/
				...
				else if($option_user_verification == 1 )//Admin Verification
				{
					...
					if(!wp_mail(sanitize_email($_POST['e_mail']), $subject, $message , $headers)){
		...
		function Unverified(){
			...
			?>
			<div class="wrap">
			  <h2><?php _e('Unverified Users', 'pie-register')?></h2>
			  <form id="verify-filter" method="post" action="">
				<div class="tablenav">
				  <div class="alignleft">
					<input onclick="return window.confirm('<?php _e('This will verify users of all types','pie-register');?>'); " value="<?php _e('Verify Checked Users','pie-register');?>" name="verifyit" class="button-secondary" type="submit">
					&nbsp;
					<input value="<?php _e('Resend Pending Payment E-mail','pie-register');?>" name="paymentl" class="button-secondary" type="submit">
					&nbsp;
					<input value="<?php _e('Resend Verification E-mail','pie-register');?>" name="emailverifyit" class="button-secondary" type="submit">
					&nbsp;
					<input value="<?php _e('Delete','pie-register');?>" name="vdeleteit" class="button-secondary delete" type="submit">
				...
				<table class="widefat">
				  <thead>
					<tr class="thead">
					  ...
					  <th><?php _e('User Name','pie-register');?></th>
					  <th><?php _e('E-mail','pie-register');?></th>
					  <th><?php _e('Registration Type','pie-register');?></th>
					  <th><?php _e('Role','pie-register');?></th>
					</tr>
				  </thead>
				  <tbody id="users" class="list:user user-list">
					<?php 
						...
						<tr id="user-1" class="<?php echo $alt;?>">
						  ...
						  <td><a href="mailto:<?php echo $un->user_email;?>" title="<?php _e('E-mail', 'pie-register'); echo ": ".$un->user_email;?>"><?php echo $un->user_email;?></a></td>
						  <td><?php
								switch($reg_type[0]){
									case "email_verify":
										_e("E-mail Verification","pie-register");
									break;
									case "admin_verify":
										_e("Admin Verification","pie-register");
									break;
									case "admin_email_verify":
										_e("E-mail & Admin Verification","pie-register");
									break;
									case "payment_verify":
										_e("Payment Verification","pie-register");
									break;
									default:
			...
			if( isset($_POST['piereg_activate_addon_license_key'], $_POST['save_addon_license_key_settings']) )
			{
				if(isset($_POST['piereg_addon_license_key_nonce']))
				{
					if(wp_verify_nonce( $_POST['piereg_addon_license_key_nonce'], 'piereg_wp_addon_license_key_nonce' )){
						...
						{
							$LK_options['api_key']			= sanitize_key($_POST['piereg_addon_license_key']);
				...
				if( ($_GET['action'] == "current_email_verify" ) && (isset($_GET['key']) && !empty($_GET['key']) ) && (isset($_GET['login']) && !empty($_GET['login']) ))
				{
					...
					if($email_verify_orignal_key == $email_verify_key)
					{
						...
						*/
						...
						$print_message = __('Invalid request. This key is no longer exists','pie-register' );
						$type = "error";
					}
				}
