...
function alnuar_add_meta_to_admin_plugins_page( $links, $file ) {
	...
	if ( strpos( $file, plugin_basename(__FILE__) ) !== false ) {
		...
		$new_links = array('<a href="'.$url.'">Settings</a>', '<a href="'.$donate_url.'">Thanks for supporting me! <img style="vertical-align:bottom;" height="30" src="'.$pluginbut.'"></a>'); //DONATE
...
function alnuar_auto_login_new_user_after_registration_options() {
		...
		<script>
			jQuery(document).ready(function($){
				$('#buttonSubmit').click(function(){ $(this).prop('value', '.....  SAVING  .....'); }); //Change Save Changes button text and disable button when clicked
			});
		</script>
		...
		<p>
			...
			<br><span style="font-size: 85%; font-style: italic;">When enabled, a new user will automatically be logged in when they register.<br>They will also receive an email telling them how to set/change their password.</span>
		</p>
		...
		<b>NOTE: These fields only work if you are using the default WordPress Registration form at /wp-login.php?action=register<br>If you are using a custom Registration form different from the default WordPress Registration form then it is recommended you leave these options unchecked/disabled.</b>
		<p>
			<?php
			$checked = "";
		...
		<br>
		<h1><?php _e( 'OTHER INDEPENDENT OPTIONS' ); ?></h1>
		<p>
			<?php
			$checked = "";
			...
			<label for="alnuar_new_user_email_text_change"><?php _e( 'Check to change the text in new users welcome email from "set your password" to "change your password" ' ) ?></label>
			<br><span style="font-size: 85%; font-style: italic;">This option only works for English language.<br>For greater customization of message (for any language) I recommend you use a plugin that allows you to customize the new user welcome email.</span>
		</p>
		<p>
			<?php
			$checked = "";
		...
		<p class="submit">
			<input class="button-primary" id="buttonSubmit" name="save" type="submit" value="<?php _e( 'Save Changes' ) ?>" />
		</p>
		</form>
